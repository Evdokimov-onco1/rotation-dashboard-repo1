...
import {
  WEEKS, UNITS, CURATORS, RESIDENTS, setScheduleData,
  type Block, weekByNum, unitById, curatorById, residentById,
  fmtD, blockRangeLabel, shortFio,
} from "./data";
import {
  errText, fetchSchedule, authPin, authLogout,
  createBlock, updateBlock, deleteBlock, assignCurator, fetchAudit,
  type AuditEntry, type BlockInput,
} from "./api";
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex gap-2">
          <Input type="password" inputMode="numeric" placeholder="PIN" value={pin}
            ...
            className="w-32 mono" />
          <Button onClick={() => void tryPin()} disabled={busy}>{busy ? "…" : "Войти"}</Button>
        </div>
...
function Matrix({ blocks, date, onCellClick }:
  ...
  const rows = RESIDENTS.map((r) => {
    ...
    for (; w <= (WEEKS.at(-1)?.num ?? 35); w++) cells.push(<td key={"e" + w} className="border border-border bg-muted/60" />);
    return (
      <tr key={r.id}>
...
function EduView({ blocks, date, unlocked, onUnlock, onLock }:
  { blocks: Block[]; date: string; unlocked: boolean; onUnlock: () => void; onLock: () => void }) {
  if (!unlocked) {
    return (
      <PinGate
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <Button variant="ghost" onClick={onLock}>Выйти</Button>
      </div>
...
const ACTION_LABELS: Record<string, string> = {
  ...
  seed_import: "загрузка графика из сида",
};

function auditBlockText(v: Partial<Block> | null): string {
  if (!v || !v.residentId) return "";
  const r = RESIDENTS.find((x) => x.id === v.residentId);
  const u = UNITS.find((x) => x.id === v.unitId);
  const c = v.curatorId ? curatorById(v.curatorId) : undefined;
  return [
    r ? shortFio(r.fio) : v.residentId,
    u ? u.short : v.unitId,
...
function AuditDialog({ onClose }: { onClose: () => void }) {
  ...
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => {
    fetchAudit().then((r) => setEntries(r.entries)).catch((e) => setErr(errText(e)));
  }, []);
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader><DialogTitle className="text-base">Журнал правок</DialogTitle></DialogHeader>
        {err && <p className="text-sm text-red-700">{err}</p>}
          ...
          <div className="max-h-[60vh] space-y-2 overflow-y-auto pr-1">
            {entries.map((e) => (
              <div key={e.id} className="border-b pb-2 text-sm last:border-0">
                ...
                <div>{auditBlockText(e.new ?? e.old) || "—"}</div>
                {e.action !== "block_create" && e.old && e.new && (
                  <div className="text-xs text-muted-foreground">было: {auditBlockText(e.old)}</div>
                )}
              </div>
            ))}
          </div>
...
function AdminView({ blocks, unlocked, onUnlock, onLock, openEditor, onBlockChanged }:
  {
    blocks: Block[]; unlocked: boolean;
    onUnlock: () => void; onLock: () => void;
  ...
  return (
    <div className="max-w-3xl space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Каждая правка сохраняется на сервере и попадает в журнал правок.
      </p>
...
function BlockDialog({ state, setState, admin, blocks, onSaved, onDeleted, onClose }:
  {
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const res = isNew ? await createBlock(payload) : await updateBlock(state.id!, payload);
      onSaved(res.block);
      onClose();
    } catch (e) {
      setMsg(errText(e));
      ...
      <DialogContent className="max-w-lg">
        ...
        {!admin ? (
          ...
          <div className="space-y-3 text-sm">
            ...
            <div>
              <label className="caps-label mb-1 block text-[10px] text-muted-foreground">Комментарий (необязательно)</label>
              <Input value={state.comment} maxLength={500}
...
export default function App() {
  ...
  useEffect(() => {
    const onVis = () => {
      if (document.visibilityState === "visible" && loaded) void load(false);
    };
    document.addEventListener("visibilitychange", onVis);
    return () => document.removeEventListener("visibilitychange", onVis);
  }, [load, loaded]);

  const unlock = () => setAdminUnlocked(true);
  const lock = async () => {
  ...
  const openEditor = (b: Block | null) =>
    setEditor(b
      ? { id: b.id, residentId: b.residentId, unitId: b.unitId, from: b.from, to: b.to, curatorId: b.curatorId, comment: b.comment ?? "" }
      : { id: null, residentId: RESIDENTS[0].id, unitId: UNITS[0].id, from: 1, to: 4, curatorId: UNITS[0].candidates[0] ?? null, comment: "" });

  const curW = useMemo(() => (today ? currentWeekNum(today) : null), [today, loaded]);
      ...
      <main className="mx-auto max-w-[1400px] px-5 py-5">
        <Tabs defaultValue="curator">
          ...
          <TabsContent value="matrix">
            <Matrix blocks={blocks} date={today} onCellClick={(b) => openEditor(b)} />
          </TabsContent>
          <TabsContent value="edu">
            <EduView blocks={blocks} date={today}
              unlocked={adminUnlocked} onUnlock={unlock} onLock={() => void lock()} />
          </TabsContent>
          <TabsContent value="admin">
            <AdminView blocks={blocks}
              unlocked={adminUnlocked} onUnlock={unlock} onLock={() => void lock()}
              openEditor={openEditor} onBlockChanged={applyBlock} />
          </TabsContent>
        </Tabs>
      </main>
