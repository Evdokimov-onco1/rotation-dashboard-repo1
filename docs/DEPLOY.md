...
Backend ищет конфиг сам: для веб-запросов — «каталог над webroot'ом / `rotation-config.php`»,
для консольных скриптов путь передаётся переменной `ROTATION_CONFIG`. Образец —
...
1. `https://rotation.mmcc-education.ru` открывается по HTTPS; в шапке «2026/27 · текущий»,
